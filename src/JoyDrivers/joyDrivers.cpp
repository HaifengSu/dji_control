...
joyStruct driverXeox(sensor_msgs::Joy msg){
  ...
  joy.buttonSelect = msg.buttons[8];
  joy.buttonStart = msg.buttons[9];

  joy.LstickHor = msg.axes[0];
  joy.LstickVer = msg.axes[1];
  joy.RstickHor = msg.axes[2];
  joy.RstickVer = msg.axes[3];
  // L2 R2 are used for yaw control
  joy.L2 = msg.axes[4];
  joy.R2 = msg.axes[5];

  /*
  // buttonLeft, right, up, down was never used
  if (msg.axes[4]>0.5) {
    joy.buttonLeft = 1;
    joy.buttonRight = 0;
