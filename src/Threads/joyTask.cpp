...
void *joyTask(void *threadID){
  ...
  while(1){
    ...
    if(localFSM.State == localFSM.MODE_POSITION_ROS){  
      //Only set these events in POSITION_ROS mode
    ...
    if(localFSM.State == localFSM.MODE_POSITION_JOY){
      ...
      // use joystick right stick as velocity input, integrate to get position 
      // command, using filterJoy
      ...
      // joystick attitude control mode
      ...
      // ROS position control mode, 
      // takes Odom as reference position
